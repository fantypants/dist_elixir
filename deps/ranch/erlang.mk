...
help::
	$(verbose) printf "%s\n" "" \
		"Escript targets:" \
		"  escript     Build an executable escript arcDistElixir" \
...
SFX_ARCDistElixir ?= $(RELX_OUTPUT_DIR)/$(RELX_REL_NAME)/$(RELX_REL_NAME)-$(RELX_REL_VSN).tar.gz
...
TMPDIR=`mktemp -d`
ARCDistElixir=`awk '/^__ARCDistElixir_BELOW__$$/ {print NR + 1; exit 0;}' $$0`
FILENAME=$$(basename $$0)
REL=$${FILENAME%.*}

tail -n+$$ARCDistElixir $$0 | tar -xzf - -C $$TMPDIR
...
__ARCDistElixir_BELOW__
endef

sfx:
	$(call render_template,sfx_stub,$(SFX_OUTPUT_FILE))
	$(gen_verbose) cat $(SFX_ARCDistElixir) >> $(SFX_OUTPUT_FILE)
